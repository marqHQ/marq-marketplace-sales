...
Public plugin marketplace for the Marq sales team. It contains one plugin (`marq-sales-suite`) with eleven skills, packaged for both Claude Code and Codex.
...
Connected and authorized as required by the selected workflow: **HubSpot**, **Gong**, **Google Calendar**, **Brandfetch**, **Marq**, and **Marq Analytics**. The discovery planner and post-call execution workflows require HubSpot access when CRM context is requested; post-call execution also requires a transcript or substantive notes. The marq-deal-acceleration skill requires HubSpot deal access, uses Gong when matched calls are available, and requires Python to run its deterministic momentum classifier. The mql-to-sqo skill also needs a browser tool signed into the HubSpot portal for line-item rebuilds. The plugin-feedback and upload-new-skill workflows need no connectors; they package the submission and post it to the contribution pipeline themselves (see [CONTRIBUTING.md](CONTRIBUTING.md)). The joke-time skill also needs no connectors. The timezone-overlap skill also needs no connectors; it requires Python 3.9+ with the IANA time zone database (on Windows, `pip install tzdata`).
